...
- **Site da apresentação final (17/09)**: deck HTML interativo (reveal.js, v4.1 de 17/09 espelhando a `Apresentacao_Final_IPB (2).pptx` do grupo — capa com mapa noturno + logo, nove bases antes da arquitetura, metodologia antes dos insights, EDA por pilar A–E com figuras interativas, Top 10 de V1/V2/V3 em tabelas, "V3 corrige o viés" com barras regionais do Top 100, clusterização e Isolation Forest no formato da PPTX; divisão de fala nas notas), mapa coroplético municipal (D3 + malha IBGE 2022) e apêndice de backup, tudo em `docs/` na branch `feature/apresentacao` e servido via GitHub Pages; dados exportados por `scripts/09` e figuras por `scripts/11`. Plano em `docs/Plano_de_Implementacao_Apresentacao_Final.md`.
...
*Última atualização (2026-09-17, v9): deck v4.1 espelhando a PPTX final do grupo (`referencias/Apresentacao_Final_IPB (2).pptx`) — 23 slides (22 visíveis; "Por que K=6" oculto como backup de banca) com capa (mapa noturno + logo), nove bases antes da arquitetura, pilares A–E com figuras interativas novas (`fig_empregos_formais`, `fig_banda_larga_regiao`, `fig_sem_agencia_uf`, `fig_top100_regioes_versoes`, `fig_anomalias_top15`), clusterização e Isolation Forest no formato da PPTX, QR do mapa no slide do mapa, fechamento "Obrigado! Dúvidas?", roteiro em 7 min 59 s.*

*Anterior (2026-09-16, v8): repaginação do deck da apresentação final (gráficos ampliados, diagrama iconográfico da stack, figuras `fig_comparacao_versoes`/`fig_escolha_k` no script 11, fontes Sora/Inter vendorizadas, classificação de presença só no apêndice A4, clustering protagonista) e roteiro do plano sincronizado.*
