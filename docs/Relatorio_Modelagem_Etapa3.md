...
**Boxplot do IPB por cluster** (figura `01_ipb_por_cluster.png`): os arquétipos ocupam faixas distintas do índice — os grupos "sem rede" concentram IPB baixo/mediano, o turismo pequeno-rico pontua alto.
...
**Resíduos — o produto de negócio:** 56 municípios com probabilidade ≥ 0,5 de "tem agência" **mas sem agência** (perfil de bancarizado não atendido). O topo da lista valida o IPB de forma gritante: **Bombinhas-SC — rank 1 do IPB V3 — com prob 0,94**, seguida de Colniza-MT (0,97), Balneário Rincão-SC, Itapeva-MG, Taguaí-SP. O scatter `prob × rank` (figura `02_residuos_rank.png`) mostra os dois produtos concordando e ainda assim gerando lista nova de oportunidades.
