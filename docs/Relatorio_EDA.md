...
Este relatório consolida os principais achados da análise exploratória de dados (EDA) do IPB. A EDA buscou:
...
Sete notebooks reprodutíveis em `notebooks/00_exploracao/` sustentam a análise:
...
Os outputs de dados (parquet e relatórios JSON) foram salvos em `data/processed/`; as figuras, em `docs/assets/figures/`, versionadas junto com este relatório.  

### 2.1 Correção e recálculo do Pix

A análise de ponta a ponta do projeto identificou um **bug crítico de duplicação** na tabela `raw_bcb_pix_transacoes`: cada mês aparecia múltiplas vezes (ex.: o mês `202606` aparecia 12 vezes), inflando artificialmente o volume Pix anualizado em aproximadamente **87%**.
...
- **Correlações**: as variáveis demográficas são relativamente independentes entre si, exceto pelo par `idhm` × `escolaridade_ensino_medio_pct` (0,78) e `populacao_urbana_pct` × `escolaridade_ensino_medio_pct` (0,70): municípios mais urbanos e com maior IDHM tendem a ser mais escolarizados. `populacao_18_35_pct` tem correlação fraca com as demais: os pilares não medem todos a mesma coisa.
...
- **Outlier no estrato pequeno**: Pacaraima (RR), com população de ~19 mil habitantes, registra R$ 516.520 de Pix per capita em 12 meses (meio milhão de reais). O valor é provavelmente efeito do comércio de fronteira.
...
- A série temporal do Pix concentra volume crescente nos meses recentes. O volume vem em trilhões de reais (R$), separado entre Pessoa Física (PF) e Pessoa Jurídica (PJ). Os dados brutos do BCB já contêm PJ, mas a camada `trusted` do IPB utiliza apenas PF como proxy de adoção digital no pilar B.
...
- **Correspondentes bancários (pilar D da V3)**: dos 216.873 correspondentes ativos do BCB (30/08/2026), **81,1% são do tipo Sede** (estabelecimento-sede do correspondente no cadastro do BCB), 15,4% Filial e 3,5% Posto, com composição homogênea entre regiões (figura `03_correspondentes_tipo_regiao.png`). A presença física no interior é majoritariamente de correspondentes, não de agências, e foi isso que motivou a ponderação por tipo (posto 1,0 / filial 0,7 / sede 0,4 / agência 1,0) adotada no pilar D da V3.
...
- **Empregos formais por 1000 hab.** (CEMPRE) correlaciona-se bem com riqueza e dinamismo: ρ = 0,79 com PIB per capita, 0,54 com Pix per capita e 0,93 com o score do pilar A, o que deu base para incluí-la no pilar A da V3.
...
3. **Gap analysis** (durante a EDA): agências sozinhas não medem mais acesso, e o BCB registra 216 mil correspondentes. Isso motivou o redesenho do Pilar D.
...
Uma tabela de apoio `analytics_ipb_comparacao` (visão larga com os 3 IPBs e 6 ranks por município) atende consultas ad hoc e a EDA.

**Mecânica da flag de turismo (V3)**, uma heurística de projeto documentada aqui:
...
Estatísticas gerais (após recalibração do gap e enriquecimento CEMPRE da V3):
...
A primeira versão do `gap_bancario_completo` era hiperbólica: `1 / (agências + correspondentes ponderados por 100k + 1)`. Como correspondentes chegam a centenas por 100k hab em cidades pequenas (densidade de lotéricas), o denominador explodia, o gap colapsava para ~0 e, pela média geométrica, o IPB de ~119 municípios zerava, todos empatados na última posição. A fórmula foi recalibrada para **gap linear** = `1 − min-max(winsorize(presença combinada))`, o mesmo padrão de inversão dos pilares D da V1/V2 (com teste de regressão dedicado em `tests/unit/test_ipb.py`). A direção do sinal se mantém (mais presença → menos gap), sem o colapso de escala.

Em setembro de 2026, o Pilar A da V3 ganhou `empregos_formais_por_1000_hab` (pessoal ocupado total do CEMPRE/IBGE, ano 2024). A variável entra porque folha de pagamento formal é o gancho produtivo nº 1 de banco/fintech (conta-salário, crédito consignado, PJ) e mede formalização que o PIB per capita não pega: capitais como São Luís e Belém têm massa salarial bancarizável com PIB pc mediano. No ranking, houve 10 trocas no Top 100. Entraram capitais e polos com mercado formal forte (São Luís 132→70, Belém 149→99, Rio Branco 146→86, Porto Velho 138→78, Fortaleza 126→75, Boa Vista 120→80, Montes Claros, Guarulhos, Patos de Minas, Gramado); saíram dormitórios de baixa formalização (Valparaíso de Goiás 68→101, Mário Campos, Florestal, Canarana-MT). Cidades conhecidas ficaram estáveis (São Paulo 18→15, Florianópolis 11→9, Campinas 44→48).

Correlação de Spearman entre os rankings: **V1×V2 = 0,842**, **V2×V3 = 0,846**, **V1×V3 = 0,840**. Com o gap hiperbólico, a V3 parecia reordenar drasticamente o ranking (V1×V3 = 0,654), mas boa parte desse efeito era artefato de calibração, não sinal. Com o gap linear e o enriquecimento CEMPRE, as três versões continuam concordando em ~85% das posições; o que separa a V3 é o eixo concorrência (presença completa vs só agências) e a dimensão PJ (formalização), não uma revolução no ranking.
...
Distribuição regional do Top 100 (V1 → V3): Sudeste 48 → 52, Sul 24 → 20, Centro-Oeste 22 → 13, Nordeste 4 → 8, Norte 2 → 7. A V3 mantém o centro de gravidade no Sudeste, dilui CO/Sul (agro/riqueza) e sobe Nordeste/Norte. Esse deslocamento vem do enriquecimento CEMPRE, que valoriza capitais e polos formais dessas regiões.

### 8.3 Alertas e sensibilidade

- **Turismo**: o Top 10 da V3 segue com cidades de evento especial (Bombinhas, Balneário Camboriú, Itapema, no litoral catarinense). A flag reduz o pilar B em até 15%; é mitigação, não solução (ver figura abaixo).
...
A partir das tabelas `raw_*`, o notebook 00b criou as seguintes features na base enriquecida `trusted_municipios_eda.parquet`:
...
As features do CEMPRE (`empregos_formais`, `unidades_locais`, `empregos_formais_por_1000_hab`, `unidades_locais_por_1000_hab`, `unidades_alojamento_alimentacao_por_1000_hab`) foram agregadas do `raw_ibge_cempre` diretamente na camada `analytics_` (tabela `analytics_ipb_v3_presenca_completa`), por serem insumo exclusivo da V3; por isso não entram em `trusted_municipios_eda`.
...
6. **IPB = 0 estrutural**: município com qualquer pilar = 0 tem IPB 0 (média geométrica). Nas 3 versões a proporção de zerados é semelhante (V1: 120, V2: 126, V3: 119 municípios). É propriedade do método (min-max + média geométrica), não bug da V3. Suavização (ex.: epsilon) ficou como decisão de método futura.
